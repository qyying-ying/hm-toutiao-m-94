  ...
  <div class="comment">
    ...
    <div class="reply-container van-hairline--top">
      ...
      <van-field v-model="value" placeholder="写评论...">
        ...
        <!-- 要么显示 提交  如果你点击了提交 首先先把 提交按钮隐藏 目的是 防止重复提交 -->
...
export default {
  ...
  methods: {
    ...
    async submit () {
      ...
      if (this.$store.state.user.token) {
        ...
        try {
          ...
          // 参数
            target: this.reply.commentId ? this.reply.commentId : this.$route.query.artId, // 文章id或者是评论id 需要根据你当前是对谁进行评论
            content: this.value,
    ...
    openReply (commentId) {
      ...
      this.reply.offset = null // 点击弹出面板的时候是新的数据
      this.reply.finished = false
      this.reply.loading = true // 主动打开加载状态
      this.getReply() // 弹出评论的评论层是 主要的去请求一次数据
    },
    // 此方法用来获取评论的评论
    // 此方法 会在第一次时执行 也会在 加载第二页 第三页时执行
    async getReply () {
      const data = await articles.getComments({
        type: 'c', // 表示评论的评论
        source: this.reply.commentId, // 获取谁的评论的评论
        offset: this.reply.offset // 赋值当前的偏移量
      })
      // 需要将数据追加到reply.list队尾
